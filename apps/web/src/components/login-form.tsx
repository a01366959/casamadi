"use client"

import { useState } from "react"
import { useRouter } from "next/navigation"
import { cn } from "@/lib/utils"
import { Button } from "@/components/ui/button"
import {
  Field,
  FieldDescription,
  FieldGroup,
  FieldLabel,
} from "@/components/ui/field"
import { Input } from "@/components/ui/input"
import { IconHotelService, IconEye, IconEyeOff } from "@tabler/icons-react"
import { createClient } from "@/lib/supabase/client"
import { toast } from "sonner"
import { ES } from "@/lib/spanish"

export function LoginForm({
  className,
  ...props
}: React.ComponentProps<"div">) {
  const [email, setEmail] = useState("")
  const [password, setPassword] = useState("")
  const [showPassword, setShowPassword] = useState(false)
  const [loading, setLoading] = useState(false)
  const router = useRouter()
  const supabase = createClient()

  const getErrorMessage = (message: string): string => {
    // Check for Supabase-specific error messages
    if (message.includes("Invalid login credentials")) {
      return ES.auth.invalidCredentials
    }
    if (message.includes("User not found") || message.includes("not exist")) {
      return ES.auth.accountNotFound
    }
    if (message.includes("Email not confirmed")) {
      return "Por favor confirma tu correo antes de iniciar sesión."
    }
    if (message.includes("Failed to fetch")) {
      return "Error de conexión. Verifica tu internet o configuración."
    }
    return message || "Ocurrió un error. Intenta de nuevo."
  }

  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault()
    
    if (!email || !password) {
      toast.warning(ES.auth.pleaseEnter)
      return
    }

    setLoading(true)

    const loginPromise = new Promise<void>((resolve, reject) => {
      supabase.auth.signInWithPassword({
        email,
        password,
      }).then(({ data, error }) => {
        setLoading(false)
        if (error) {
          const userMessage = getErrorMessage(error.message)
          reject(new Error(userMessage))
        } else if (data.session) {
          setEmail("")
          setPassword("")
          setTimeout(() => router.push("/dashboard"), 500)
          resolve()
        }
      }).catch((err) => {
        setLoading(false)
        const message = err instanceof Error ? err.message : "Ocurrió un error"
        const userMessage = getErrorMessage(message)
        reject(new Error(userMessage))
      })
    })

    toast.promise(loginPromise, {
      loading: ES.auth.signing || "Iniciando sesión...",
      success: "¡Bienvenido a Casa Mädi!",
      error: (err) => {
        const message = err instanceof Error ? err.message : "Credenciales inválidas"
        toast.warning(message)
        return message
      },
    })
  }

  return (
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <form onSubmit={handleLogin}>
        <FieldGroup>
          <div className="flex flex-col items-center gap-2 text-center">
            <div className="flex flex-col items-center gap-2 font-medium">
              <div className="flex size-8 items-center justify-center rounded-md">
                <IconHotelService className="size-6" />
              </div>
              <span className="sr-only">Casa Mädi</span>
            </div>
            <h1 className="text-xl font-bold">Casa Mädi</h1>
            <FieldDescription>
              {ES.auth.staffLogin}
            </FieldDescription>
          </div>

          <Field>
            <FieldLabel htmlFor="email">{ES.auth.email}</FieldLabel>
            <Input
              id="email"
              type="email"
              placeholder="staff@hotel.com"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              disabled={loading}
              required
            />
          </Field>

          <Field>
            <FieldLabel htmlFor="password">{ES.auth.password}</FieldLabel>
            <div className="relative">
              <Input
                id="password"
                type={showPassword ? "text" : "password"}
                placeholder="••••••••"
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                disabled={loading}
                required
                className="pr-10"
              />
              <Button
                type="button"
                variant="ghost"
                size="sm"
                className="absolute right-0 top-0 h-full px-3 py-2 hover:bg-transparent"
                onClick={() => setShowPassword(!showPassword)}
                disabled={loading}
              >
                {showPassword ? (
                  <IconEyeOff className="size-4 text-muted-foreground" />
                ) : (
                  <IconEye className="size-4 text-muted-foreground" />
                )}
              </Button>
            </div>
          </Field>

          <Field>
            <Button type="submit" disabled={loading} className="w-full">
              {loading ? ES.auth.signing : ES.auth.signIn}
            </Button>
          </Field>
          <Field className="grid gap-4 sm:grid-cols-2">
            <Button variant="outline" type="button">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                <path
                  d="M12.152 6.896c-.948 0-2.415-1.078-3.96-1.04-2.04.027-3.91 1.183-4.961 3.014-2.117 3.675-.546 9.103 1.519 12.09 1.013 1.454 2.208 3.09 3.792 3.039 1.52-.065 2.09-.987 3.935-.987 1.831 0 2.35.987 3.96.948 1.637-.026 2.676-1.48 3.676-2.948 1.156-1.688 1.636-3.325 1.662-3.415-.039-.013-3.182-1.221-3.22-4.857-.026-3.04 2.48-4.494 2.597-4.559-1.429-2.09-3.623-2.324-4.39-2.376-2-.156-3.675 1.09-4.61 1.09zM15.53 3.83c.843-1.012 1.4-2.427 1.245-3.83-1.207.052-2.662.805-3.532 1.818-.78.896-1.454 2.338-1.273 3.714 1.338.104 2.715-.688 3.559-1.701"
                  fill="currentColor"
                />
              </svg>
              Continue with Apple
            </Button>
            <Button variant="outline" type="button">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                <path
                  d="M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .307 5.387.307 12s5.56 12 12.173 12c3.573 0 6.267-1.173 8.373-3.36 2.16-2.16 2.84-5.213 2.84-7.667 0-.76-.053-1.467-.173-2.053H12.48z"
                  fill="currentColor"
                />
              </svg>
              Continue with Google
            </Button>
          </Field>
        </FieldGroup>
      </form>
      <FieldDescription className="px-6 text-center">
        By clicking continue, you agree to our <a href="#">Terms of Service</a>{" "}
        and <a href="#">Privacy Policy</a>.
      </FieldDescription>
    </div>
  )
}
